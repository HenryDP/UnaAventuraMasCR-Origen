  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const formattedData = {
        ...
        price_national: (data.price_national !== undefined && data.price_national !== '') ? Number(data.price_national) : null,
        price_foreigner: (data.price_foreigner !== undefined && data.price_foreigner !== '') ? Number(data.price_foreigner) : null,
        images: typeof data.images === 'string' ? data.images.split('\n').map((i: string) => i.trim()).filter(Boolean) : (Array.isArray(data.images) ? data.images : []),
        included: typeof data.included === 'string' ? data.included.split('\n').map((i: string) => i.trim()).filter(Boolean) : (Array.isArray(data.included) ? data.included : []),
        recommendations: typeof data.recommendations === 'string' ? data.recommendations.split('\n').map((i: string) => i.trim()).filter(Boolean) : (Array.isArray(data.recommendations) ? data.recommendations : []),
        pickupLocations: typeof data.pickupLocations === 'string' ? data.pickupLocations.split('\n').map((i: string) => i.trim()).filter(Boolean) : (Array.isArray(data.pickupLocations) ? data.pickupLocations : []),
      };
